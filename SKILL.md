...
description: 金谷园饺子馆信息查询与在线排队取号。通过金谷园官方 API 查询基础店铺信息（餐厅介绍、门店、营业时间、外卖配送、Wi-Fi）、生饺子打包、最新动态、当前排队状态、单点到店预估、午市/晚市排队事实与策略建议、到店自取叫号下单、菜品配方、店长推荐菜；内置真实排队动作仅用于在线取号、本人排队进度查询、取消排队。
version: 3.0.3
alwaysApply: false
keywords:
  - 金谷园
...
仅当用户明确要求登录金谷园、验证金谷园身份，或调用需登录能力（如登录测试消息）时使用。服务端未开启时会返回错误，如实诚告知用户该能力尚未上线。与美团授权（jgy.cjs queue）完全无关，两套凭证互不影响。

命令入口：`node <skill_dir>/scripts/jgy.cjs <command>`

1. **新会话先查状态**：`jgy.cjs auth-status`。本地凭证自动恢复登录态，`authenticated=true` 时无需重新短信，直接调用业务。
2. **登录流程**（auth-status 未登录时）：向用户说明需手机号验证 → 问手机号 → `jgy.cjs auth-start --phone "<手机号>"` → 告知脱敏号码已发送验证码 → 请用户只提供本次验证码 → 用安全标准输入完成：`printf '%s' "<验证码>" | node <skill_dir>/scripts/jgy.cjs auth-complete --login-id <login_id> --stdin` → 成功后继续原任务。
3. **需登录测试能力**：`jgy.cjs call authenticated-test`；公开查询可用 `jgy.cjs call <capability>`（带结构化参数时加 `--args '<json>'`）。
4. **登出**：`jgy.cjs logout`（撤销服务端授权并清本地凭证）。
5. **换账号**：用户要求用另一个手机号登录时，必须先 `jgy.cjs logout` 成功再走登录流程；禁止在已登录状态下直接给新号发码（本地单账号凭证，覆盖前不撤销会在服务端留下悬挂授权）。

**硬红线**：验证码只能走标准输入，绝不作为命令行参数（`--code` 会被拒绝），不写进回复正文；不展示任何 token；不复述完整手机号（只用脱敏号）；登录失败遵守服务端冷却时间，不自动重试超过一次；`/clear`、新会话不影响本地登录态。
...
3. 根据返回的 JSON 结果向用户展示奖品或安慰信息
4. 如果结果建议登录（next_action 包含 login），询问用户是否登录保存
5. 用户同意后：按上文登录流程完成手机号验证，再执行 `node <skill_dir>/scripts/jgy.cjs bind_prize --slug <SLUG>` 将奖品绑定到账号，返回 `PRIZE_ISSUED_BOUND` 即绑定成功；之后换设备登录同一手机号也能找回
6. 用户想查已领奖品（如"查一下我的金谷园奖品"）：执行 `node <skill_dir>/scripts/jgy.cjs list_my_prizes`
