...
var require_passport = __commonJS({
  "scripts/passport.js"(exports2, module2) {
    ...
    var CLI_PATH = path.resolve(__dirname, "vendor/pt-passport/dist/index.js");
    var PASSPORT_BUNDLED = path.resolve(__dirname, "vendor/pt-passport/bundled_modules");
    ...
    function createPassport({ run = defaultRun, homeDir = os.homedir(), tmpDir = os.tmpdir() } = {}) {
      ...
      async function invoke(args, { missingCacheIsNull = false } = {}) {
        ...
        if (process.platform !== "win32") fs.chmodSync(cacheDir, 448);
        let completed;
        try {
    ...
    module2.exports = { createPassport, PASSPORT_BUNDLED };
  }
});
...
var require_auth_poll_bg = __commonJS({
  "scripts/lib/auth-poll-bg.js"(exports2, module2) {
    ...
    var { PASSPORT_BUNDLED } = require_passport();
    var { isAuthRunId } = require_auth_run();
  ...
  "scripts/queue.js"(exports2, module2) {
    ...
    async function beginAuthorization(deps, { force = false } = {}) {
      ...
      try {
        ...
        data.agentHint = AUTH_INLINE_QR_HINT + "\u540E\u53F0\u672A\u542F\u52A8\uFF1A\u5C55\u793A\u540E auth-poll --background\uFF0C\u518D\u7528 auth-status\u3002";
      }
      const message = [
        "\u9700\u8981\u5B8C\u6210\u7F8E\u56E2\u6388\u6743\u3002",
...
// scripts/lib/credential-store.js
var require_credential_store = __commonJS({
  "scripts/lib/credential-store.js"(exports2, module2) {
    "use strict";
    var os = require("node:os");
    var path = require("node:path");
    var fs = require("node:fs");
    ...
    function atomicWrite(file, contents) {
      ensureDir();
      const tmp = `${file}.${process.pid}.${crypto.randomBytes(6).toString("hex")}.tmp`;
      fs.writeFileSync(tmp, contents, { mode: 384 });
      try {
    ...
    function write(creds) {
      const payload = { version: CRED_VERSION, updated_at: (/* @__PURE__ */ new Date()).toISOString(), ...creds };
    ...
    function fileMode(file = credPath()) {
      try {
        return (fs.statSync(file).mode & 511).toString(8);
      } catch {
        return null;
      }
    }
...
// scripts/lib/jgy-auth.js
var require_jgy_auth = __commonJS({
  "scripts/lib/jgy-auth.js"(exports2, module2) {
    "use strict";
    var fs = require("node:fs");
    var path = require("node:path");
    var store = require_credential_store();
    var { createApiClient, ApiError } = require_jgy_api();
    var CLIENT_ID = process.env.JGY_CLIENT_ID || "jinguyuan-dumpling-skill";
    var RESOURCE = process.env.JGY_RESOURCE || "https://mcp.jinguyuan.cloud";
    var REFRESH_SKEW_MS = 12e4;
    ...
    function createAuth({ api = createApiClient(), now = () => Date.now() } = {}) {
      ...
      async function logout() {
        ...
        if (creds && creds.refresh_token) {
          try {
            await api.revoke({ token: creds.refresh_token });
          } catch {
          }
        }
        store.clear();
      ...
      function persistTokens(res, prev = {}) {
        const expiresInMs = (Number(res.expires_in) || 3600) * 1e3;
        return store.write({
          client_id: CLIENT_ID,
          device_id: deviceId,
      ...
      async function withLock(fn) {
        ...
        store.baseDir();
        fs.mkdirSync(store.baseDir(), { recursive: true, mode: 448 });
        let held = false;
    ...
    module2.exports = { createAuth, AuthError, CLIENT_ID, RESOURCE };
  }
});

// scripts/lib/lottery-api.js
var require_lottery_api = __commonJS({
  "scripts/lib/lottery-api.js"(exports2, module2) {
    "use strict";
    ...
    function createLotteryApi({ bearer = null } = {}) {
      ...
      function getPrizeDetail(claimId, { claimToken } = {}) {
        const headers = claimToken ? claimTokenHeaders(claimToken) : bearerHeaders();
        return apiCall({
          method: "GET",
    ...
    module2.exports = { createLotteryApi };
  }
});

// scripts/lib/lottery-claims.js
var require_lottery_claims = __commonJS({
  "scripts/lib/lottery-claims.js"(exports2, module2) {
    "use strict";
    var crypto = require("node:crypto");
    var fs = require("node:fs");
    var os = require("node:os");
    var path = require("node:path");
    var STORE_DIR = path.join(os.homedir(), ".jinguyuan");
    var STORE_PATH = path.join(STORE_DIR, "anonymous-claims.json");
    function ensureDir() {
      if (!fs.existsSync(STORE_DIR)) {
        fs.mkdirSync(STORE_DIR, { recursive: true, mode: 448 });
      }
    }
    function loadAll() {
      ensureDir();
    ...
    module2.exports = { getOrCreateClaim, getAllClaims, saveClaim, getClaimByToken, updateClaim, removeClaim };
  }
});
...
var require_jgy = __commonJS({
  "scripts/jgy.js"(exports2, module2) {
    "use strict";
    var { createAuth, AuthError } = require_jgy_auth();
    var { createApiClient } = require_jgy_api();
    var { createLotteryApi } = require_lottery_api();
    var claims = require_lottery_claims();
    var { ok, err, emit } = require_output();
    var SLUG_RE = /^[A-HJ-NP-Z2-9]{6,12}$/;
    ...
    async function main() {
      ...
      switch (command) {
        ...
        case "bind_prize": {
          const slug = args.slug || args._[0];
          if (!slug) return emit(err("invalid_command", "\u7528\u6CD5: bind_prize --slug <SLUG>"));
          const result = await bindPrize(slug, auth);
          return emit(result);
        }
        case "call": {
          const capability = args._[0];
          if (capability === "authenticated-test") {
            const data2 = await auth.callAuthenticatedTest();
            return emit(ok({ data: data2 }));
          }
    ...
    async function getLottery(slug, auth) {
      ...
      if (status.authenticated) {
        try {
          bearerToken = await auth.getAccessToken();
        } catch {
        }
      }
      ...
      return ok({ result, next_action: nextAction.length ? nextAction : void 0 });
    }
    ...
    async function bindPrize(slug, auth) {
      const status = auth.authStatus();
      if (!status.authenticated) {
        return err("auth_required", "\u7ED1\u5B9A\u5956\u54C1\u9700\u8981\u5148\u767B\u5F55\u3002\u8BF7\u5148\u6267\u884C auth-start \u5B8C\u6210\u624B\u673A\u53F7\u9A8C\u8BC1\u3002");
      }
      const allClaims = claims.getAllClaims();
      const claim = allClaims[slug];
      if (!claim) {
        return err("claim_not_found", `\u672A\u627E\u5230 slug=${slug} \u7684\u672C\u5730\u9886\u53D6\u51ED\u8BC1\u3002`);
      }
      if (claim.claim_state === "issued_bound") {
        return ok({ message: "\u8BE5\u5956\u54C1\u5DF2\u7ED1\u5B9A\u8D26\u53F7\uFF0C\u65E0\u9700\u91CD\u590D\u64CD\u4F5C\u3002", slug });
      }
      const bearerToken = await auth.getAccessToken();
      const lotteryApi = createLotteryApi({ bearer: bearerToken });
      const result = await lotteryApi.bindPrize(claim.claim_token);
      if (result.ok) {
        claims.updateClaim(slug, { claim_state: "issued_bound" });
      }
      return result;
    }
    module2.exports = { runCli, getLottery, listMyPrizes, bindPrize };
  }
});
